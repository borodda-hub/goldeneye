...
- **B5 — Cross-asset portability.** *maps-to:* ROADMAP #9 + CALIBRATION architecture principles. *status:* BUILD. *depends-on:* platform stable single-asset (B1–B2). *effort:* M.
  Make deadband, regime bands, and the registry **per-asset-class config**, not hardcodes; de-commoditize `registry.py`; light up one second asset class (equities or rates). **DoD:** second asset class runs the full loop with no commodity hardcodes; gate green.
