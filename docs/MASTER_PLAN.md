...
- **D1 — Implied-vs-realized vol probe.** *maps-to:* the Phase-30 vol edge, extended against a market price. *status:* ⬆️ **PROMISING (2026-07-25 revisit; pre-registered tie-breaker)** — G2 premium-timing passes **2-of-3 pairs** (crude USO/OVX +5.6±3.8; equities SPY/^VIX +2.3±2.1 with har_log beating VIX as an RV predictor >2 SE); gold fails; EWMA arm fails everywhere. **Next: D1b design review** (a "vol rich/cheap" research view — own pre-registered gate before any ship). Detail: `PHASE_D1_PLAN.md §revisit`. *effort:* M (probes done).
