...
- **D1 — Implied-vs-realized vol probe.** *maps-to:* the Phase-30 vol edge, extended against a market price. *status:* ⬆️ **PROMISING (2026-07-25 revisit; pre-registered tie-breaker)** — G2 premium-timing passes **2-of-3 pairs** (crude USO/OVX +5.6±3.8; equities SPY/^VIX +2.3±2.1 with har_log beating VIX as an RV predictor >2 SE); gold fails; EWMA arm fails everywhere. **D1b SHIPPED 2026-07-27:** the "Vol vs Market" Signal Lab card — DESCRIPTIVE comparison only (claim split, live ship-gate, no signal chip), served by the SAME function the probe validates; promotion/demotion rules pre-registered. Detail: `PHASE_D1_PLAN.md §D1b`. *effort:* M (done).
...
- **D4 — Event-window selective abstention (storage-day).** *maps-to:* the queued selective-abstention quick-win + C3's surprise proxy. *status:* 🚫 **PROBED 2026-07-25 — INSUFFICIENT-N + premise absent** (release-day corr with the seasonal-norm surprise is a tight zero on n=559: the market's expectation is analyst consensus, not the 5y norm — details `PHASE_D4_PLAN.md §verdict`). No view built; re-entry needs a real expectation source (paid consensus, or weather-adjusted via a matured D5 archive). *effort:* M (probe done).
