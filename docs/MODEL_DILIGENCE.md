...
| **Vol-premium timing** (D1: forecast-RV vs implied-vol spread) | **real-OOS** ⬆️ PROMISING (2026-07-25, revisit) | **Tested, not crowned.** Pre-registered probe, ~11y weekly, n_eff≈131/pair. Premium replicates on all three pairs (table stakes). The *timing* claim (har_log-vs-IV spread → subsequent premium): **G2 passes 2-of-3 with consistent direction** — USO/OVX (low−high +5.6 ± 3.8) and SPY/^VIX (+2.32 ± 2.07, and G1: har_log beats VIX as an RV predictor by >2 SE); GLD/GVZ fails; the EWMA robustness arm fails everywhere. Upgraded PARTIAL→PROMISING per the tie-breaker rule pre-registered BEFORE the revisit run. **D1b surfaced (2026-07-27) as a DESCRIPTIVE comparison only** — the Signal Lab "Vol vs Market" card (CL/GC/ES; no free NG IV index → honestly absent) serves the SAME `analyze_pair` the probe validates, gated by a live per-request ship-gate (our forecast must not be worse than the market's own); tercile stats display as tested-not-crowned; gold's line is labeled FAILED; promotion + demotion rules pre-registered in `PHASE_D1_PLAN.md §D1b` | `seeds/validate_d1_vol_premium.py` (thin harness over `services/vol_premium.py` — the surface cannot diverge from the test); rules pre-registered in `PHASE_D1_PLAN.md` |
