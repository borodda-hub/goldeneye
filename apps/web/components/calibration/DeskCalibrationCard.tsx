...
import type { DeskAnalyst } from "@/lib/api";
  ...
  return (
    <section className="card-interactive border border-line-1 bg-surface-1 p-3 flex flex-col gap-2.5">
        ...
        <table className="w-full text-xs font-mono">
          <thead>
            <tr className="text-[9px] uppercase tracking-widest text-ink-4 text-left">
              ...
              <th className="font-normal pr-2 text-right">Hit</th>
      ...
      <p className="text-[9px] text-ink-4 font-mono leading-relaxed">
        ...
        conviction) measures whether an analyst's confidence is reliable — the
        skill signal; <span className="text-ink-3">Hit</span> is the raw
        outcome, which luck contaminates. Scores are withheld until{" "}
        {data?.min_resolved ?? 10} decisions resolve. Descriptive
        decision-quality diagnostics, not advice.
      </p>
    </section>
  );
}
