...
| **Signal Lab** | 5-model ensemble (moving average, prophet, volatility regime, weighted-factor composite, trained logistic-regression) with agreement, input diversity, confidence rationale, per-model supporting/contradicting factors, LLM explanation, history table with server-side hit/miss scoring. |
